...
/* ─ hash router: "#/w/h/<token>?..." ─ */
function parseHash() {
  const raw = (window.location.hash || "#").slice(1);
  const [path, query] = raw.split("?");
  const parts = (path || "").split("/").filter(Boolean);
  const token = parts[0] === "w" && parts[1] === "h" ? parts[2] : null;
  return { token, qs: new URLSearchParams(query || "") };
}
function useHashRoute() {
  const [st, setSt] = React.useState(parseHash());
  React.useEffect(() => {
    const onHash = () => setSt(parseHash());
...
async function getOwnerByToken(token) {
  const { data, error } = await supabase
    .from("overlay_tokens")
    .select("user_id, disabled, expires_at")
    .eq("token", token)
    .maybeSingle();
  if (error) throw error;
  if (!data) throw new Error("Token inválido.");
  if (data.disabled) throw new Error("Este link foi desativado.");
  if (data.expires_at && new Date(data.expires_at) < new Date())
    throw new Error("Este link expirou.");
  return data.user_id;
}

/* ─ owner -> hunt ativo (numberId) ─ */
...
/* ─ Página ─ */
export default function HuntWidgetByToken() {
  const { token, qs } = useHashRoute();
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!token) throw new Error("Token em falta (#/w/h/<token>).");

        // 1) token -> owner
        const owner = await getOwnerByToken(token);

        // 2) hunt ativo desse owner
        const numberId = await resolveActiveNumberId(owner);
        if (!numberId) throw new Error("Não foi encontrado um hunt ativo para este jogador.");

        // 3) guard opcional: confirma owner do hunt
        const { hunt } = await getHuntByNumberId(Number(numberId));
        const hOwner = hunt?.user_id ?? hunt?.owner_id ?? hunt?.profile_id ?? hunt?.created_by;
        if (hOwner && hOwner !== owner) throw new Error("Hunt não pertence a este jogador.");

        // 4) slots
        const { slots: s } = await listHuntSlots({ numberId: Number(numberId) });
        if (!alive) return;
  ...
  if (err) {
    return (
      <div style={{display:"grid",placeItems:"center",height:"100vh",color:"#fca5a5",background:"#0b1020",
        fontFamily:"'Rubik',ui-sans-serif,system-ui,-apple-system,'Segoe UI',Roboto,Arial",padding:16,textAlign:"center"}}>
        {err}
      </div>
    );
  }
...
/* ─ UI do overlay (igual ao hunt-widget) ─ */
      ...
      {layout === "grid" ? (
        <div style={{display:"grid",gridTemplateColumns:"repeat(8,minmax(0,1fr))",gap}}>
          {slots.slice(0, 16).map((s, i) => (
            <Card key={s.id ?? i} s={s} i={i} width="100%" cardH={opts.cardH || 160} opts={opts} />
          ))}
        </div>
      ) : (
        <div style={{display:"flex",alignItems:"center",justifyContent:justify,height:"100%",overflow:"hidden"}}>
          <div
            style={{
              display:"flex",
              gap,
          ...
          >
            {[...slots, ...slots].map((s, i) => (
              <Card key={`${s.id ?? "x"}-${i}`} s={s} i={i % (slots.length || 1)} width={cardW} cardH={opts.cardH || 160} opts={opts} />
            ))}
          </div>
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      {opts.showSuper && isSuper && (
        <div
          style={{
            position:"absolute",zIndex:5,top:6,[infoRight ? "left" : "right"]:6,
            padding:"2px 8px",borderRadius:999,fontSize:10,fontWeight:700,letterSpacing:.4,
            background:anyToRgba(opts.superTagColor || "#e879f9", 0.95),color:opts.superTextColor || "#120614",
          }}
        >
          SUPER
        </div>
      ...
      {showName && captionIsBar && (
        <div style={{ position: "absolute", left: 8, right: 8, bottom: 8 }}>
          <div
            title={s?.name || ""}
            style={{ padding:"8px 10px", borderRadius:10, border:"1px solid rgba(255,255,255,.18)", background:"rgba(0,0,0,.45)", boxShadow:"0 10px 30px rgba(0,0,0,.45)" }}
          >
      ...
      {showName && captionIsFloat && (
        <div style={{ position:"absolute", left:8, right:8, bottom:8, pointerEvents:"none" }}>
          <div style={{ fontWeight:600, textShadow:"0 2px 6px rgba(0,0,0,.8)" }}>{s?.name || "—"}</div>
          {canShowBet && opts.betStyle === "inline" && (

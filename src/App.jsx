...
function ThemeIconToggle() {
  ...
  return (
    <button
      onClick={toggle}
      className={cn("p-2 rounded-xl", isDark ? "hover:bg-white/5 text-white/80" : "hover:bg-zinc-100 text-zinc-700")}
...
const WidgetAmountWon = ({ amount = 0, currency = "€" }) => {
  const { isDark } = useTheme();
  return (
    <div className={cn("p-4 rounded-2xl border shadow-sm flex items-center justify-between", isDark ? "border-white/10 bg-white/5" : "border-zinc-200 bg-white")}>
      <div className={cn("text-sm", isDark ? "text-white/60" : "text-zinc-600")}>Amount won</div>
      <div className="text-xl font-bold tracking-tight">{currency} {Number(amount).toLocaleString(undefined,{minimumFractionDigits:2, maximumFractionDigits:2})}</div>
    </div>
  );
};
const WidgetAverageBet = ({ size = 0, currency = "€" }) => {
  const { isDark } = useTheme();
  return (
    <div className={cn("p-4 rounded-2xl border shadow-sm flex items-center justify-between", isDark ? "border-white/10 bg-white/5" : "border-zinc-200 bg-white")}>
      <div className={cn("text-sm", isDark ? "text-white/60" : "text-zinc-600")}>Average bet</div>
      <div className="text-xl font-semibold">{currency} {Number(size).toFixed(2)}</div>
    </div>
  );
};
const WidgetRemaining = ({ count = 0 }) => {
  const { isDark } = useTheme();
  return (
    <div className={cn("p-4 rounded-2xl border shadow-sm flex items-center justify-between", isDark ? "border-white/10 bg-white/5" : "border-zinc-200 bg-white")}>
      <div className={cn("text-sm", isDark ? "text-white/60" : "text-zinc-600")}>To open</div>
      <div className="text-xl font-semibold">{count}</div>
    </div>
  );
};
const WidgetProgress = ({ value = 0 }) => {
  const { isDark } = useTheme();
  return (
    <div className={cn("p-4 rounded-2xl border shadow-sm", isDark ? "border-white/10 bg-white/5" : "border-zinc-200 bg-white")}>
      <div className={cn("text-sm mb-2", isDark ? "text-white/60" : "text-zinc-600")}>Progress</div>
...
function AuthCallback({ onDone }) {
  ...
  useEffect(() => {
    const run = async () => {
      const p = getHashParams();
      if (p.error) { onDone && onDone(); return; }
      history.replaceState(null, "", window.location.pathname + window.location.search + "#/home");
      onDone && onDone();
    };
    run();
  ...
  if (huntDetailMatch) {
    ...
    content = (
      <>
        ...
        {route === "auth" && <AuthCallback onDone={() => navigate("home")} />}

...
export default function WidgetOverlay() {
  const { id } = useParams(); // battle id (string)
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        if (l) setLayout((prev) => ({ ...prev, ...l }));
        if (o) setOpts((prev) => ({ ...prev, ...o }));
  ...
  return (
    ...
    >
      ...
      {opts.bonusDock === "right" ? (
        <div className="flex items-center justify-between gap-2">
          <div
            ...
            style={{ background: theme.badgeBg, border: `${theme.badgeBorderWidth}px solid ${theme.badgeBorder}`, borderRadius: theme.pillRadius, fontWeight: theme.fontWeight }}
          >
            <span>Best of</span>
      ...
      {/* total */}
      <div className={opts.totalJustify === "left" ? "mt-6 flex justify-start" : opts.totalJustify === "right" ? "mt-6 flex justify-end" : "mt-6 flex justify-center"}>
        <div

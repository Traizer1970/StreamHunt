...
// Battles page — JSX version (no TypeScript). Matches your Tournaments styling.
...
const Badge = ({ children, tone = "zinc" }) => (
  <span
    className={[
      ...
      tone === "zinc" && "bg-white/8 text-zinc-200 ring-1 ring-white/10",
      tone === "cyan" && "bg-cyan-500/15 text-cyan-300 ring-1 ring-cyan-500/30",
      tone === "violet" && "bg-violet-500/15 text-violet-300 ring-1 ring-violet-500/30",
      tone === "emerald" && "bg-emerald-500/15 text-emerald-300 ring-1 ring-emerald-500/30",
      tone === "rose" && "bg-rose-500/15 text-rose-300 ring-1 ring-rose-500/30",
    ].join(" ")}
  >
    {children}
  </span>
...
const IconBtn = ({ label, onClick, tone = "zinc" }) => (
  <button
    onClick={onClick}
    className={[
      "inline-flex h-9 w-9 items-center justify-center rounded-xl ring-1 transition",
      tone === "zinc" && "bg-white/5 text-white/70 ring-white/10 hover:bg-white/10",
      tone === "cyan" && "bg-cyan-500/15 text-cyan-300 ring-cyan-500/30 hover:bg-cyan-500/20",
      tone === "rose" && "bg-rose-500/15 text-rose-300 ring-rose-500/30 hover:bg-rose-500/20",
    ].join(" ")}
    aria-label={label}
    title={label}
  >
    {label === "View" && (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 5c-7 0-11 7-11 7s4 7 11 7 11-7 11-7-4-7-11-7zm0 12a5 5 0 1 1 0-10 5 5 0 0 1 0 10z"/></svg>
    )}
    {label === "Edit" && (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zm2.92 2.83H5v-0.92l9.06-9.06.92.92L5.92 20.08zM20.71 7.04a1.003 1.003 0 0 0 0-1.42l-2.34-2.34a1.003 1.003 0 0 0-1.42 0l-1.83 1.83 3.75 3.75 1.84-1.82z"/></svg>
    )}
    {label === "Delete" && (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M6 7h12l-1 14H7L6 7zm9-3 1 2H8l1-2h6z"/></svg>
    )}
  </button>
);

// Very simple bar chart with divs
const BarChart = ({ data }) => {
    ...
    <div className="grid grid-cols-[40px_1fr] gap-2">
      <div className="flex flex-col justify-between text-xs text-white/60">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i}>{Math.round((max / 4) * (4 - i))}</div>
        ))}
      </div>
...
export default function BattlesPage() {
  ...
  const filtered = useMemo(() => (
    battles.filter(b => b.title.toLowerCase().includes(query.toLowerCase()))
  ), [query]);
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="h-10 w-64 rounded-xl bg-white/5 pl-9 pr-3 text-sm placeholder:text-white/40 ring-1 ring-white/10 focus:outline-none"
              />
            </div>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-12 space-y-4 lg:col-span-4">
            <Card title="Top Player (wins)">
              <div className="flex items-center gap-3">
                <div className="grid h-10 w-10 place-items-center rounded-full bg-cyan-500 text-slate-950">{topPlayer.name[0]?.toUpperCase() || "P"}</div>
            ...
            <Card title="Last battle winner">
              <div className="flex items-center gap-3">
                <div className="grid h-10 w-10 place-items-center rounded-full bg-cyan-500 text-slate-950">{lastWinner.player[0]?.toUpperCase() || "P"}</div>
                <div>
                ...
                <div key={b.id} className="flex items-center px-4 py-3 hover:bg-white/5">
                  ...
                  <div className="grid flex-1 grid-cols-[2fr_1fr_1fr_1fr_140px] items-center gap-2 px-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {b.prizes.map((p, i) => (
                        <Badge key={i} tone="violet">🏅 {currency}{p.toFixed(2)}</Badge>
                      ))}
                    </div>
                    <div>
